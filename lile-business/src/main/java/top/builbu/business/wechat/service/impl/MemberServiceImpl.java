...
public class MemberServiceImpl implements MemberService{
    ...
    @Override
    public ResultDO<MemberDTO> selectByOpenId(String openId){
    	ResultDO<MemberDTO> resultDo =null;
    	if(null != openId && !"".equals(openId)){
    		MemberDTO result = memberMapper.selectByOpenId(openId);
    		if(null != result){
    			resultDo = new ResultDO<>(result); 
    		}else{
    			resultDo = new ResultDO<>(BaseResultCode.COMMON_MESSAGE_CHENGGONG,Boolean.TRUE);
    		}
    	}else{
    		resultDo = new ResultDO<>(BaseResultCode.COMMON_WRONG_PARAMS,Boolean.FALSE);
    	}
    ...
    @Override
    public ResultDO<?> updateByOpenId(Member member){
    	ResultDO<?> result = null;
    	Integer rowId = memberMapper.updateByOpenId(member);
    	if(null != rowId && rowId > 0){
    		result = new ResultDO<>(BaseResultCode.COMMON_MESSAGE_CHENGGONG,Boolean.TRUE);
    	}else{
    		result = new ResultDO<>(BaseResultCode.COMMON_MESSAGE_LOSE,Boolean.FALSE);
